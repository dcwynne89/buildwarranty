...
async function downloadWarranty() {
  ...
  try {
    ...
    const payload = {
      ...
      coverage:            data.coverageDescription,
      exclusions:          data.exclusions,
      claim_process:       data.claimProcess,
    ...
    if (!result.success || !result.pdf) {
      showToast(result.error || "Failed to generate warranty. Please try again.", "error");
      return;
    }
    ...
    link.href     = url;
    link.download = `warranty-${data.warranty.number || "001"}.pdf`;
    link.click();
    URL.revokeObjectURL(url);

    showToast("✓ Warranty PDF downloaded!", "success");
  } catch (err) {
    console.error(err);
    ...
    isGenerating = false;
    btn.disabled = false;
    btn.textContent = "⬇ Download Warranty PDF";
  }
}
